...
int main(int argc, char** argv) {
    ...
    int children_ready_notif_pipe[2];
    if(pipe(children_ready_notif_pipe) == -1) {
    ...
    for (int i = 0; i < N; i++) {
        switch (pid = fork()) {
            case -1:
                ...
                close(children_ready_notif_pipe[0]);
                close(children_ready_notif_pipe[1]);
                free(children);
                return EXIT_FAILURE;
            case 0:
                ...
                write(children_ready_notif_pipe[1], "k", 1);
    ...
    char r;
    for (int i = 0; i < N; i++) {
        read(children_ready_notif_pipe[0], &r, 1);
    }
